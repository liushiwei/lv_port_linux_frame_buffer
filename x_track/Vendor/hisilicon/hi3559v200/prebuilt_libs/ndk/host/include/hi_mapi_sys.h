...
#define HI_MAPI_SYS_ENO_REG_MSG \
    HI_MAPI_DEF_ERR(HI_MAPI_MOD_SYS, MAPI_EN_ERR_LEVEL_ERROR, MAPI_EN_ERR_NO_REGISTER_MSG)
...
/**
 * @register vqe module.
 * @param[in] pstAudioVqeCfg HI_MAPI_AUDIO_VQEATTR_S*:pointer of register vqe module attribute.
 * @return 0  register vqe module success.
 * @return non 0  register vqe module fail.
 */

HI_S32 HI_MAPI_Sys_RegisterVqeModule(HI_MAPI_AUDIO_VQEATTR_S *pstAudioVqeCfg);
...
/**
 * @brief get vb block from vb.
 * @param[out] poolHdl HI_HANDLE *:The block from vb's handle.
 * @param[in] pu64PhyAddr HI_U64*:The physical address of block.
 * @param[out] pVirtAddr HI_VOID *:The virtual address of block.
 * @param[in] blkSize HI_U64:The length of block.
 * @return 0  get buffer successful.
 * @return non 0  get buffer fail.
 */
HI_S32 HI_MAPI_SYS_GetVbBlock(HI_HANDLE *poolHdl, HI_U64* phyAddr, HI_VOID **virAddr, HI_U64 blkSize, const HI_CHAR *mmzName);

/**
 * @brief Free buffer from MMZ.
 * @param[in] pu64PhyAddr HI_U64*:The physical address of block.
 * @param[in] pVirtAddr HI_VOID *:The virtual address of block.
 * @param[in] blkSize HI_U64:The length of block.
 * @return 0  release buffer successful.
 * @return non 0  release buffer fail.
 */
 HI_S32 HI_MAPI_SYS_ReleaseVbBlock(HI_U64 phyAddr, HI_VOID* virAddr, HI_U64 blkSize);
