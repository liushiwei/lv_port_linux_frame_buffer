...
} VENC_HEAD_INFO_TYPE_E;

typedef struct hiVENC_HEAD_INFO_S
{
    VENC_HEAD_INFO_TYPE_E enHeadInfoType;
    HI_CHAR azHeadInfo[MEDIA_VENC_HEAD_INFO_BUFF_LEN];
    HI_U32 u32Length;
} VENC_HEAD_INFO_S;
...
typedef enum {
    HI_MAPI_VENC_PIC_RECEIVE_SINGLE = 0,
    HI_MAPI_VENC_PIC_RECEIVE_MULTI,
    HI_MAPI_VENC_PIC_RECEIVE_BUTT
} HI_MAPI_PicReceiveMode;
...
typedef struct hiMAPI_VENC_ATTR_JPEG_S {
    ...
    HI_MAPI_PicReceiveMode  receiveMode;      /* config the receive mode */
