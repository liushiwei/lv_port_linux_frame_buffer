...
typedef struct HiStateInfo {
    ...
    HI_S32 (*processMessage)(HI_MESSAGE_S *msg, HI_MW_PTR argv, HI_STATE_ID *stateID);
    HI_MW_PTR argv;                             /* User private, used by processMessage */
}HI_STATE_S;

/* hfsm event enum */
typedef enum HiHfsmEventE {
...
typedef HI_S32 (*HI_HFSM_EVENT_CALLBACK_FN)(HI_MW_PTR hfsmHandle, const HI_HFSM_EVENT_INFO_S *eventInfo);

typedef struct HiHfsmAttr {
 ...
 */
HI_S32 HI_HFSM_Create(HI_HFSM_ATTR_S *fsmAttr, HI_MW_PTR *hfsm);
...
/**
  ...
  *    @return     0 success, non-zero error code
 */
HI_S32 HI_HFSM_Destroy(HI_MW_PTR hfsm);
...
/**
  ...
  *    @return     0 success, non-zero error code
 */
HI_S32 HI_HFSM_AddState(HI_MW_PTR hfsm, HI_STATE_S *state, HI_STATE_S *parent);
...
/**
  ...
  *    @return     0 success, non-zero error code
 */
HI_S32 HI_HFSM_SetInitialState(HI_MW_PTR hfsm, HI_CHAR *stateName);
...
/**
  ...
  *    get current state
  *    @param[in]  hfsm: point of FHSM handle
  *    @param[out]  state: current state point
  *    @return     0 success, non-zero error code
 */
HI_S32 HI_HFSM_GetCurrentState(HI_MW_PTR hfsm, HI_STATE_S *state);
...
/**
  ...
  *    @return     0 success, non-zero error code
 */
HI_S32 HI_HFSM_Start(HI_MW_PTR hfsm);
...
/**
  ...
  *    @return     0 success, non-zero error code
 */
HI_S32 HI_HFSM_Stop(HI_MW_PTR hfsm);
...
/**
  ...
  *    @return     0 success, non-zero error code
 */
HI_S32 HI_HFSM_SendAsyncMessage(HI_MW_PTR hfsm, HI_MESSAGE_S *msg);
