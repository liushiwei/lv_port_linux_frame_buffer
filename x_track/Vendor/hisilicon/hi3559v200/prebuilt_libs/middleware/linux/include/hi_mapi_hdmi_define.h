...
typedef struct hiMAPI_HDMI_SINKCAPABILITY_S {
    ...
    HI_BOOL bSupportHdmi;   /* < Whether the HDMI is supported by the device. If the HDMI
                                 is not supported by the device, the device is DVI. */
    ...
    HI_BOOL abIntfSync[VO_OUTPUT_BUTT];    /* < Video capability set.HI_TRUE: This display sync is supported.
...
typedef struct hiMAPIHDMI_ATTR_S {
    ...
    HI_BOOL bEnableVideo;      /* < Whether to output video.The value must be HI_TRUE.
                                    If the value is HI_FALSE, the HDMI is forcibly set to HI_TRUE. */
    VO_INTF_SYNC_E enHdmiSync; /* < Video norm. This value of the video norm
                                    must be consistent with the norm of the video output. */
    ...
    HI_U32 u32PixClk;          /* < Pixclk of enVideoFmt(unit is kHz).(This param is valid only when
                                    enVideoFmt is HI_HDMI_VIDEO_FMT_VESA_CUSTOMER_DEFINE) */
} HI_MAPI_HDMI_ATTR_S;
